...
class EasyApplyBot:
    ...
    def applications_loop(self, position, location):
        ...
        print(
            "bwhat is the value of our error?? ", self.browser.set_window_position(1, 1)
        )
    ...
    def send_resume(self) -> bool:
        ...
        try:
            ...
            stuck_counter = {"index": 0, "counter": 0}
                    ...
                    if is_present(error_locator):
                        for element in self.browser.find_elements(
                            ...
                            if "Please enter a valid answer" in text:
                                ...
                                break
                    if button:
                        if stuck_counter["index"] == i:
                            stuck_counter["counter"] = stuck_counter["counter"] + 1
                        else:
                            stuck_counter["index"] = i
                            stuck_counter["counter"] = 0

                        if stuck_counter["counter"] >= 12:
                            # need to leave this application so we are not stuck on a form field (12 was chosen randomly) |DL
                            log.info(
                                "Couldn't Fill Out All Required Field/s, Discarding Application"
                            )
                            submitted = True
    ...
    # Discard Application if we can't fillout the form
    def discard_application(self) -> None:
        dismiss_locator = (
            By.CSS_SELECTOR,
            "button[aria-label='Dismiss']",
        )
        dismiss_button: None = self.wait.until(
            EC.element_to_be_clickable(dismiss_locator)
        )
        ...
        discard_button: None = self.wait.until(
            EC.element_to_be_clickable(discard_locater)
        )
